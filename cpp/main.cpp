...
double sigma_second_der(double x) {
    return 2 * cos(2 * x);
}
...
double *get_euler_trajectory(double *fbm, int length, double t, double alpha) {
    ...
    for (int i = 1; i < length; ++i) {
        currentValue = currentValue - alpha * currentValue * increment + fbm[i - 1];
...
double dvbyt_integrand(double s, void *p) {
    ...
    double r = c(h) * exp(-alpha * t) * pow(u, 0.5 - h) * pow(s, h - 0.5) * exp(alpha * s) * pow(s_minus_u, h - 1.5);
    return r;
}
...
double s_array_integrand(double x, void *p) {
    ...
    return sigma(step_y_x) * sigma_der(step_y_x) *
           dvbyt(params->h, params->alpha, params->k * params->t / params->n, x);
}
...
double *s_array(double *y, int n, double t, double alpha, double h) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        F.params = &params;
        double localResult;
        gsl_integration_cquad(&F, 0.001, t, 0, 0.001, w, &localResult, nullptr, nullptr);
...
double density_inner_integrand(double v, void *p) {
    struct inner_density_integrand_params *params = (struct inner_density_integrand_params *) p;
...
double dvbeta_inner_integrand(double q, void *p) {
    ...
    double step_y_tau = step(params->y, params->n, params->t / params->n, params->tau);
    return -4 * pow(params->eta, 2) * step_s_q *
           (pow(sigma_der(step_y_tau), 2) + sigma(step_y_tau) * sigma_second_der(step_y_tau))
           * dvbyt(params->h, params->alpha, params->v, params->tau) * dvbyt(params->h, params->alpha, q, params->tau);
}
...
double density(double *y, double *s, double *sbm_incr, int n, double t, double h, double alpha) {
    ...
    for (int k = 1; k < n; ++k) {
        ...
        integrand.params = &params;
        gsl_integration_cquad(&integrand, k * t / n, t, ABS_ERR, REL_ERR, w, &integrationResult, nullptr, nullptr);
        first += pow((k + 1) * t / n, 0.5 - h) * integrationResult * sbm_incr[k];
    }
    first *= 2 * eta * c(h);
    double sndsn;
    gsl_function integrand;
